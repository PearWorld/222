...
static int _vsnprintf(out_fct_type out, char* buffer, const size_t maxlen, const char* format, va_list va)
{
            ...
            case 'p' :
            {
    ...
    #if defined(PRINTF_SUPPORT_LONG_LONG)
                const bool is_ll = sizeof(uint32_t) == sizeof(long long);
                if (is_ll)
                {
                    idx = _ntoa_long_long(out, buffer, idx, maxlen, (uint32_t)va_arg(va, void*),
                ...
                else
                {
                     ...
                     //idx = _ntoa_long(out, buffer, idx, maxlen, (unsigned long)((uintptr_t)va_arg(va, void*)),
                                      //false, 16U, precision, width, flags);// initial
